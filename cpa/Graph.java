...
public class Graph implements GraphInterface{
	...
	private void setIncomingDegreeRecursively(GraphNode node) {
		...
		for (GraphEdge outgoingEdge : node.getOutgoingEdges()) {
			GraphNode nextNode = outgoingEdge.getEndPoint();
			if (nextNode.getDegree() == 0) {
				setIncomingDegreeRecursively(nextNode);
			}
		}
	}
	...
	private QueueInterface<GraphNode> topologicalSort( ) {
		...
		while (!tempQueue.isEmpty()) {
			GraphNode node = tempQueue.dequeue();
			resultQueue.enqueue(node);
			for (GraphEdge outgoingEdge : node.getOutgoingEdges()) {
				GraphNode nextNode = outgoingEdge.getEndPoint();
				nextNode.setDegree(nextNode.getDegree() - 1);
				if (nextNode.getDegree() == 0) {
	...
	private void computeEarliestCompletionTime( ) {
		...
		while (!sortedNodes.isEmpty()) {
			...
			int precedentEct = 0;
			for (GraphEdge incomingEdge : node.getIncomingEdges()) {
				GraphNode precedentNode = incomingEdge.getStartPoint();
				if (precedentNode.EarliestCompletionTime() > precedentEct) {
					precedentEct = precedentNode.EarliestCompletionTime();
				}
			}

      int taskTime = node.getIncomingEdges().get(1).getTaskDuration();

      node.setEarliestCompletionTime(precedentEct + taskTime);
